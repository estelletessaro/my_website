---
title: "London Bike Rentals over Time"
date: '2017-10-31T22:26:13-05:00'
description: 
draft: no
image: londonbike.jpg
keywords: ''
slug: homework2b
categories:
- ''
...
TfL releases data on how many bikes were hired every single day. We can get the latest data by running the following
...
```{r tfl_month_year_grid, echo=TRUE, out.width="60%"}
knitr::include_graphics(here::here("images", "tfl_distributions_monthly.jpg"), error = FALSE)
```
...
Let's recreate the following graph:

```{r tfl_absolute_monthly_change, echo=FALSE, out.width="60%"}
knitr::include_graphics(here::here("images", "tfl_monthly.jpg"), error = FALSE)
```

```{r}
#Let's create dataframes to filter out certain years and organise them by months.
...
monthly_average <- bike %>% 
  ...
  summarise(monthly_average = mean(bikes_hired),
            year = unique(year))
#Let's now join these dataframes.
full_monthly_averages <- left_join(monthly_average, general_monthly_average, by="month")

#Lastly, let's recreate the plots using ggplot2(), and specifically geom_line and geom_ribbon. Let's also facet_wrap these by year so that we can see yearly changes.
...
The second one looks at percentage changes from the expected level of weekly rentals. Let's also recreate it.

```{r tfl_percent_change, echo=FALSE, out.width="60%"}
knitr::include_graphics(here::here("images", "tfl_weekly.jpg"), error = FALSE)
```

```{r}
#Let's create a dataframe to filter out certain dates and organize them by week.
expected_bike_pw <- bike %>%
  filter(day >= as.Date("2016-1-1") & day <= as.Date("2019-12-31")) %>%
  group_by(week) %>% 
  summarise(expected_rentals = mean(bikes_hired))

#Next, we organize by year and week and join both dataframes.
bike_pw <- bike %>%
